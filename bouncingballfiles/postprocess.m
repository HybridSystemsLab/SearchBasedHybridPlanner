...
for i = 2:1:max(m_edgelistbw, m_edgelistfw)
    if (i <= m_edgelistfw)
        plot(Edgelistfw{i,3}(1,1), Edgelistfw{i,3}(2,1),'k*');
        hold on
        plot(Edgelistfw{i,3}(1,end), Edgelistfw{i,3}(2,end),'k*');
        hold on
        p1 = plot(Edgelistfw{i,3}(1,:), Edgelistfw{i,3}(2,:),'k-', 'DisplayName','Forward propagation');
        hold on
    end
    if (i <= m_edgelistbw)
        plot(Edgelistbw{i,3}(1,1), Edgelistbw{i,3}(2,1),'m*');
        hold on
        plot(Edgelistbw{i,3}(1,end), Edgelistbw{i,3}(2,end),'m*');
        hold on
        p2 = plot(Edgelistbw{i,3}(1,:), Edgelistbw{i,3}(2,:),'m-', 'DisplayName','Backward propagation');
        hold on
    end
    xlim([-18, 20]);
    ylim([-25,15]);
...
for i = 1: n_pathfw - 1
    for j = 2:m_edgelistfw
        if Edgelistfw{j,1} == endpointpathfw(1, i) && Edgelistfw{j,2} == endpointpathfw(1, i  + 1)
            p3 = plot(Edgelistfw{j,3}(1, :), Edgelistfw{j,3}(2, :),'r-', 'LineWidth', 3);
            hold on
        end
    end
end
...
for i = 1: n_pathbw - 1
    for j = 2:m_edgelistbw
        if Edgelistbw{j,1} == endpointpathbw(1, i) && Edgelistbw{j,2} == endpointpathbw(1, i  + 1)
            plot(Edgelistbw{j,3}(1, :), Edgelistbw{j,3}(2, :),'r-', 'LineWidth', 3);
        end
    end
end
